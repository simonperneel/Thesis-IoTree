 ...
 *
 *       Adapted version from DRAMCO Research Group *
 *       v1.0: DRAMCO
 *       v2.0: changed setupABP to use ABP joins after modem is completely powered down
 *       		settings are already configured with the first OTAA JOIN
...
RN2483_Status_t RN2483_SetupOTAA(LoRaSettings_t settings, char * receiveBuffer, uint8_t bufferSize){
	...
	status = RN2483_SetDeviceAddress(settings.deviceAddress, receiveBuffer, bufferSize); //blank ABP credentials in preparation of
		if(status != MAC_OK){
			return status;
		}
...
/* commands below are commented out: already done with OTAA join */
//	RN2483_Status_t status = RN2483_MacReset(receiveBuffer, bufferSize);
//	if(status != MAC_OK){
//		return status;
//	}
//	RN2483_Status_t status = RN2483_SetDeviceAddress(settings.deviceAddress, receiveBuffer, bufferSize);
//	if(status != MAC_OK){
//		return status;
//	}
//	status = RN2483_SetNetworkSessionKey(settings.networkSessionKey, receiveBuffer, bufferSize);
...
RN2483_Status_t RN2483_PauseMac(char * receiveBuffer, uint8_t bufferSize){
	sprintf(commandBuffer, "mac pause\r\n");
	return RN2483_ProcessMacCommand(receiveBuffer, bufferSize, false);
}
...
RN2483_Status_t RN2483_Reset(char * receiveBuffer, uint8_t bufferSize){
	sprintf(commandBuffer, "sys reset\r\n");
	return RN2483_ProcessMacCommand(receiveBuffer, bufferSize, false);
}
